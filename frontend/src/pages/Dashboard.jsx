...
const Dashboard = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [statsRes, regionsRes, tiposRes, controlRes] = await Promise.all([
        ...
        axios.get(`${API}/tipos-instalacion`),
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(`${API}/search?sucursal=${encodeURIComponent(searchTerm)}`);
    ...
    <div className="min-h-screen bg-[#020617] text-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <section data-testid="charts-section" className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {/* Bar Chart */}
          <Card className="lg:col-span-2 bg-slate-950/50 border-slate-800 hover:border-slate-700 transition-colors animate-fade-in">
          ...
          {/* Pie Chart */}
          <Card className="bg-slate-950/50 border-slate-800 hover:border-slate-700 transition-colors animate-fade-in-delay-1">
            <CardHeader className="pb-2">
